...
| Module | Role |
|--------|------|
| `app/main.py` | FastAPI app, CORS (localhost:3055), startup model preload, calibrator loop |
...
All API calls target `process.env.NEXT_PUBLIC_API_URL` (default `http://localhost:8055`).
...
- [ ] Lock CORS to real frontend origin(s) for production (currently localhost:3055).
