...
4. **A `resume_id`** — most endpoints need one. Get it by uploading (B.1) or
   ...
   ```powershell
   $RID = (Invoke-RestMethod -Uri http://localhost:8004/api/resume/scaffold -Method Post -ContentType application/json -Body '{}').resume_id
   $RID
   ```
   All commands below assume `$RID` holds a valid id and `$API = "http://localhost:8004"`.
   ```powershell
...
Run frontend at `http://localhost:3004`. Walk each page; for each, check the
